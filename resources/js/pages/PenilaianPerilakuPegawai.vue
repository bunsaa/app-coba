...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Penilaian Perilaku', href: '/penilaian-perilaku-pegawai' },
  { title: 'Penilaian Perilaku Pegawai', href: '/penilaian-perilaku-pegawai' },
];
...
const unsurPerilaku = [
  ...
  { key: 'loyal', label: 'Loyal', pedoman: 'Memegang teguh ideologi Pancasila, Undang-Undang Dasar Negara Republik Indonesia Tahun 1945, NKRI serta pemerintahan yang sah' },
...
// Generate daftar bulan: 12 bulan ke belakang + bulan sekarang
...
function tampilkan() {
  router.get('/penilaian-perilaku-pegawai', {
...
const filteredList = computed(() => {
  ...
  return props.pegawaiList.filter(p =>
    p.name.toLowerCase().includes(q) ||
    p.unit_nama.toLowerCase().includes(q) ||
    (roleLabels[p.role] || p.role).toLowerCase().includes(q)
  );
});
...
function openView(pegawai: PegawaiItem) {
  ...
  axios.post('/penilaian-perilaku-pegawai/show', {
    user_id: pegawai.id,
    periode: filterPeriode.value,
  }).then(res => {
    viewPenilaian.value = res.data.penilaian;
...
function openEdit(pegawai: PegawaiItem) {
  ...
  // Load existing penilaian jika ada
  axios.post('/penilaian-perilaku-pegawai/show', {
    user_id: pegawai.id,
    periode: filterPeriode.value,
  }).then(res => {
    if (res.data.penilaian) {
      const p = res.data.penilaian;
      formEdit.value = {
...
function submitPenilaian() {
  ...
  // Validasi: semua unsur harus diisi
  ...
  if (penilaianId) {
    // Update existing
    router.put(`/penilaian-perilaku-pegawai/${penilaianId}`, formEdit.value, {
      preserveScroll: true,
      onSuccess: () => {
    ...
    // Create new
    router.post('/penilaian-perilaku-pegawai', {
      user_id: editPegawai.value.id,
      periode: filterPeriode.value,
      ...formEdit.value,
    }, {
      preserveScroll: true,
...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-4 p-4 xl:p-6">
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 -mt-3">Daftar pegawai yang dibawah struktur</p>
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-end gap-3 flex-wrap">
        ...
        <button
          @click="tampilkan"
          class="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-medium text-white hover:bg-indigo-700 shadow-md transition-colors"
        >
          Tampilkan
        </button>
        <a
          v-if="isAdmin"
          :href="'/penilaian-perilaku-pegawai/export?periode=' + filterPeriode"
          class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-5 py-2 text-sm font-medium text-white hover:bg-emerald-700 shadow-md transition-colors"
        >
          <Download :size="16" />
          Download Excel
        </a>
      ...
      <div v-if="showEdit" class="fixed inset-0 z-[9999] grid place-items-center bg-black/40 p-4" @click.self="showEdit = false">
        <div class="w-full max-w-3xl rounded-xl bg-white p-6 shadow-2xl dark:bg-gray-900 max-h-[90vh] overflow-y-auto">
          ...
          <div v-if="editPegawai" class="mb-4 space-y-2">
            ...
            <div class="grid grid-cols-4 gap-2 text-sm">
              <span class="font-medium text-gray-600 dark:text-gray-400">Jabatan</span>
              <span class="col-span-3 px-3 py-1.5 bg-gray-100 dark:bg-gray-800 rounded text-gray-900 dark:text-gray-100">{{ roleLabels[editPegawai.role] || editPegawai.role }}</span>
            </div>
            <div class="grid grid-cols-4 gap-2 text-sm">
              <span class="font-medium text-gray-600 dark:text-gray-400">Status Pegawai</span>
